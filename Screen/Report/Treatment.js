...
import { ScrollView, Center, Radio, Button, View, VStack, Text } from "native-base";
...
import { TherapyOptions, status_Option, Drug_Taken_Options, pills_taken_Options, missed_doses_Reason } from "../../Common/Options";
...
const Treatment = (props) => {
	...
	const initialValues = () => {
		if (mode === Operation_Mode.create) {
			...
			return {
				...
				Therapy_Other: initialState.Therapy_Other,
	...
	const formik = useFormik({
		...
		validationSchema: Yup.object().shape({
			...
			Therapy_Other: Yup.string().when("Therapy", {
				is: () => {
					return !_.isEmpty(formik.values, "Therapy") && _.get(formik.values, "Therapy").id === "Other";
				},
				then: Yup.string().required("This Field is required when Therapy is 'Other'"),
			}),
			Received: Yup.string().required("Please specify on whether user retrieved the treatment"),
			Chemoprophylaxis_taken: Yup.string().required("Please specify on whether user taken the malaria chmoprophylaxis"),
			Drug_taken: Yup.object().required("Drug taken is required"),
			Drug_taken_Other: Yup.string().when("Drug_taken", {
				is: () => {
					return !_.isEmpty(formik.values, "Drug_taken") && _.get(formik.values, "Drug_taken").id === "Other";
				},
				then: Yup.string().required("This is needed when Drug Taken is 'other'"),
			}),
			pills_taken: Yup.object().required("Pills taken is required"),
			pills_taken_Other: Yup.string().when("pills_taken", {
				is: () => {
					return !_.isEmpty(formik.values, "pills_taken") && _.get(formik.values, "pills_taken").id === "Other";
				},
				then: Yup.string().required("This is needed when pills taken is 'other'"),
			}),
			missed_dose_reason: Yup.object().when("pills_taken", {
				is: () => {
					return _.get(formik.values, "pills_taken").id !== "Yes, missed no dose";
				},
				then: Yup.object().required("Missed dose reason is needed"),
			}),
			Side_Effect: Yup.string().when("missed_dose_reason", {
				is: () => {
					return !_.isEmpty(formik.values, "missed_dose_reason") && _.get(formik.values, "missed_dose_reason").id === "Had a side effect";
				},
		...
		onSubmit: (values) => {
			...
			Treatment_copy.Therapy_Other = Treatment_copy.Therapy === "Other" ? Treatment_copy.Therapy_Other : "";

			Treatment_copy.pills_taken = Treatment_copy.pills_taken.id;
			Treatment_copy.missed_dose_reason = Treatment_copy.pills_taken === "Yes, missed no dose" ? "" : Treatment_copy.missed_dose_reason;
			Treatment_copy.missed_dose_reason = Treatment_copy.missed_dose_reason === "" ? "" : Treatment_copy.missed_dose_reason.id;
			...
			<Center>
				<Card_Component heading={"Management"}>
					<FormSigleSelect
						...
						onChange={onChange}
					/>
					{_.get(formik.values, "Therapy").id === "Other" && (
						<FormInputField
							formik={formik}
							Label={"Please Specify (Therapy for this attack)"}
							id="Therapy_Other"
							isRequired={true}
						/>
					)}
				...
				<Card_Component heading={"Malaria Chemoprophylaxis"}>
					...
					<FormSigleSelect
						...
						id="pills_taken"
					/>
					{_.get(formik.values, "pills_taken") && _.get(formik.values, "pills_taken").id === "Other" && (
						<FormInputField
							formik={formik}
					...
					{_.get(formik.values, "pills_taken") && _.get(formik.values, "pills_taken").id !== "Yes, missed no dose" && (
						<FormSigleSelect
							formik={formik}
					...
					{_.get(formik.values, "missed_dose_reason").id == "Had a side effect" && (
						<FormInputField
				...
				>
					{mode === Operation_Mode.create ? "Submit Treatment" : mode === Operation_Mode.edit ? "Update Treatment" : null}
				</Button>
				<FancyAlert
					visible={visible}
					icon={
